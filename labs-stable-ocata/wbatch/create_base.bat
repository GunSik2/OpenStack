...
ECHO %time% Looking for %IMGDIR%\ubuntu-16.04.4-server-amd64.iso
IF EXIST %IMGDIR%\ubuntu-16.04.4-server-amd64.iso goto got_install_iso

ECHO.
ECHO ubuntu-16.04.4-server-amd64.iso not found in %IMGDIR%.
ECHO.
ECHO Trying to download the install ISO from
ECHO http://releases.ubuntu.com/16.04/ubuntu-16.04.4-server-amd64.iso
ECHO.
ECHO Expect this to take several minutes or longer, depending on your
ECHO Internet connection.
ECHO.
cscript /nologo %TOOLSDIR%\downloader.js http://releases.ubuntu.com/16.04/ubuntu-16.04.4-server-amd64.iso
RENAME downloaded.bin ubuntu-16.04.4-server-amd64.iso
MOVE ubuntu-16.04.4-server-amd64.iso %IMGDIR%
IF EXIST %IMGDIR%\ubuntu-16.04.4-server-amd64.iso goto got_install_iso
ECHO.
ECHO ubuntu-16.04.4-server-amd64.iso still not found in %IMGDIR%.
ECHO Aborting.
ECHO.
...
:got_install_iso
ECHO.
ECHO %time% Found %IMGDIR%\ubuntu-16.04.4-server-amd64.iso
ECHO.
...
"%VBM%" modifyvm base --memory 512
IF %errorlevel% NEQ 0 GOTO :vbm_error

ECHO VBoxManage storageattach base --storagectl IDE --port 0 --device 0 --type dvddrive --medium %IMGDIR%\ubuntu-16.04.4-server-amd64.iso
"%VBM%" storageattach base --storagectl IDE --port 0 --device 0 --type dvddrive --medium %IMGDIR%\ubuntu-16.04.4-server-amd64.iso
IF %errorlevel% NEQ 0 GOTO :vbm_error
...
ECHO VBoxManage controlvm base keyboardputscancode 07 87
"%VBM%" controlvm base keyboardputscancode 07 87
IF %errorlevel% NEQ 0 GOTO :vbm_error
...
MOVE /y %IMGDIR%\tmp-disk.vdi %IMGDIR%\base-vbadd-ocata-ubuntu-16.04-server-amd64.vdi
ECHO.
